...
async function handler(req: NextRequest) {
  ...
  if (req.method === "GET") {
    ...
    const SUMMARY_FIELDS =
      "id,title,status,intent,priority,project,assigned_agent,human_input_needed,parent_task_id,created_by,updated_at,created_at";
    ...
    if (intent && VALID_INTENTS.includes(intent as TaskIntent))
    ...
    else if (humanInput === "false")
      query = query.eq("human_input_needed", false);
  ...
  if (req.method === "POST") {
    ...
    if (body.metadata) {
      ...
      if (vals.some((v: any) => v !== null && typeof v === "object")) {
        return error(
          "metadata values must be flat (string, number, boolean, or null). Use context for nested data.",
        );
      }
    }
    ...
    const insert = {
      ...
      next_run_at: body.recurrence
        ? computeNextRun(body.recurrence)?.toISOString() || null
        : null,
      user_id: auth.data.userId,
    };

    const { data: task, error: dbErr } = await db
      .from("tasks")
      .insert(insert)
      .select()
      .single();
